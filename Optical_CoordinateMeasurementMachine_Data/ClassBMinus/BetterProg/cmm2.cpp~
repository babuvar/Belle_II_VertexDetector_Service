...
Double_t theta,BestTheta,ErrDist,BestErrDist=100000000,a=DATUM.ParA,b=DATUM.ParB;
TVector3 Rot(-a,-b,1);
for(i=0;i<16;i++){VM.at(i)=DATUM.A.at(i);}

//for(theta=-(TMath::Pi()/20);theta <= (TMath::Pi()/20);theta=theta+0.0001){
for(theta=0;theta <= 0;theta=theta+0.0001){
for(i=0;i<16;i++){
VD.at(i).RotateZ(theta);
Rotate3D(VD.at(i),Rot);
VD.at(i)=VD.at(i)+CGM;

cout<<"M:   ";VM.at(i).Print();
cout<<"D:   ";VD.at(i).Print();
cout<<endl;
}//i
}//theta
