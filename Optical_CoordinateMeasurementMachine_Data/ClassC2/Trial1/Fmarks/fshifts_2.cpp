//Here we use the real design centres of 'F' marks instead of values given by Shimizu-san
#include<iostream>
#include<fstream>
#include<cmath>
using namespace std;
...
vec3 UnitX1,UnitY1,B1;
B.set(571.164,212.1);B1.set(571.164,212.1,106.279167875);//z is set from bw mount block measurement
A.set(425.167,214.3);A1.set(425.167,214.3,106.279167875);//z is set from bw mount block measurement
C=B.sub(A);
UnitX=C.GetUnit();
...
vec3 UnitX1,UnitY1,B1;
B.set(554.906,221.184);B1.set(554.906,221.184,106.279167875);//z is set from bw mount block measurement
A.set(408.899,221.008);A1.set(408.899,221.008,106.279167875);//z is set from bw mount block measurement
C=B.sub(A);
UnitX=C.GetUnit();
UnitY=UnitX.Rotate(pi/2);
...
vec3 temp;
for(i=0;i<=15;i++){
temp=U[i].sub(B1);x=temp.dot(UnitX1);y=temp.dot(UnitY1);U[i].set(x,y,temp.Z);
temp=V[i].sub(B1);x=temp.dot(UnitX1);y=temp.dot(UnitY1);V[i].set(x,y,temp.Z);
temp=W[i].sub(B1);x=temp.dot(UnitX1);y=temp.dot(UnitY1);W[i].set(x,y,temp.Z);
}
...
L1p.set(-52.631,293.272,-13.021);
L2p.set(-42.843,167.997,12.812);
L3p.set(-103.157,167.997,12.812);
L4p.set(-93.369,293.272,-13.021);
M1p.set(-43.6,165.05,13.020);
M2p.set(-43.6,40.95,13.020);
M3p.set(-102.4,40.95,13.020);
M4p.set(-102.4,165.05,13.020);
N1p.set(-43.6,40.05,13.020);
N2p.set(-43.6,-84.05,13.020);
N3p.set(-102.4,-84.05,13.020);
N4p.set(-102.4,40.05,13.020);
*/
...
vec3 L1p,L2p,L3p,L4p,M1p,M2p,M3p,M4p,N1p,N2p,N3p,N4p;
L1p.set(-52.631,288.605,-13.021);
L2p.set(-42.843,166.022,12.812);
L3p.set(-103.157,166.022,12.812);
L4p.set(-93.369,288.605,-13.021);
M1p.set(-43.6,165.05,13.020);
M2p.set(-43.6,40.95,13.020);
M3p.set(-102.4,40.95,13.020);
M4p.set(-102.4,165.05,13.020);
