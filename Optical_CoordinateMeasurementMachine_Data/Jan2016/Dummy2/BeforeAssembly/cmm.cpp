...
void FitPlane(data & DATUM, Int_t option);
...
//Slant, Tilt Angles
if(FWRand.Data_Status==1){FitPlane(FWRand,1);} else{if(FW_F.Data_Status==1){FitPlane(FW_F,1);}else{cout<<"No data on FW sensor found"<<endl;}}
if(CERand.Data_Status==1){FitPlane(CERand,2);} else{if(CE_F.Data_Status==1){FitPlane(CE_F,2);}else{cout<<"No data on CE sensor found"<<endl;}}
if(BWRand.Data_Status==1){FitPlane(BWRand,3);} else{if(BW_F.Data_Status==1){FitPlane(BW_F,3);}else{cout<<"No data on BW sensor found"<<endl;}}
...
//Redoing the fit
if(FWRand.Data_Status==1){FitPlane(FWRand,1);} else{FitPlane(FW_F,1);}
}//if
else{
...
if(option == 1){FW_F.ParA=f->GetParameter(1); FW_F.ParB=f->GetParameter(2);}
else if(option == 2){CE_F.ParA=f->GetParameter(1); CE_F.ParB=f->GetParameter(2);}
else if(option == 3){BW_F.ParA=f->GetParameter(1); BW_F.ParB=f->GetParameter(2);}
}//else
...
//Cross-Check
cout<<"---------------------------------------------------------------------------------------------------"<<endl;
cout<<(VM.at(0)-VM.at(1)).Mag()<<"\t"<<(VM.at(1)-VM.at(2)).Mag()<<"\t"<<(VM.at(2)-VM.at(3)).Mag()<<"\t"<<(VM.at(3)-VM.at(0)).Mag()<<"\t"<<(VM.at(0)-VM.at(2)).Mag()<<"\t"<<(VM.at(1)-VM.at(3)).Mag()<<endl;

cout<<(VD.at(0)-VD.at(1)).Mag()<<"\t"<<(VD.at(1)-VD.at(2)).Mag()<<"\t"<<(VD.at(2)-VD.at(3)).Mag()<<"\t"<<(VD.at(3)-VD.at(0)).Mag()<<"\t"<<(VD.at(0)-VD.at(2)).Mag()<<"\t"<<(VD.at(1)-VD.at(3)).Mag()<<endl;
cout<<"---------------------------------------------------------------------------------------------------"<<endl;
*/
...
TVector3 L(62.05,0,0),L1(62.6382,0,0),M(0.1,0,0),M1(0.075,0,0),N1(0,29.25,0),N2(0,29.55,0),O(0,0.1,0),O1(0,0.075,0);
TVector3 NS1(0,20.219,0),NS2(0,20.519,0),NL1(0,30.007,0),NL2(0,30.307,0);



if(option==1){
VD.at(0)=L1+M1-NS1-O1;
VD.at(1)=L1-M1-NS1-O1;
VD.at(2)=L1-M1-NS1+O1;
VD.at(3)=L1+M1-NS1+O1;
VD.at(4)=-L1+M1-NL1-O1;
VD.at(5)=-L1-M1-NL1-O1;
VD.at(6)=-L1-M1-NL1+O1;
VD.at(7)=-L1+M1-NL1+O1;
VD.at(8)=-L1+M1+NL2-O1;
VD.at(9)=-L1-M1+NL2-O1;
VD.at(10)=-L1-M1+NL2+O1;
VD.at(11)=-L1+M1+NL2+O1;
VD.at(12)=L1+M1+NS2-O1;
VD.at(13)=L1-M1+NS2-O1;
VD.at(14)=L1-M1+NS2+O1;
VD.at(15)=L1+M1+NS2+O1;}
...
TVector3 Rot(-a,-b,1),Diff,AsymYTrans(0,0.15,0);
for(i=0;i<16;i++){VM.at(i)=DATUM.A.at(i);}

//for(theta=-(TMath::Pi()/20);theta <= (TMath::Pi()/20);theta=theta+0.0002){
for(theta=-0.01;theta <=0.01;theta=theta+0.0002){
ErrDist=0;
for(i=0;i<16;i++){
VD1.at(i)=VD.at(i);
VD1.at(i)=VD1.at(i)-AsymYTrans;
