...
TVector3 xplane(0.9785089851,0,-0.20620418539), yplane(0,1,0), zplane(0.20620418539,0,0.9785089851), v2(365.073000,0.150000,-0.104500);

//DATUM.A.at(0).SetXYZ(426.365,-20.219,-13.021); DATUM.A.at(1).SetXYZ(303.781,-30.007,12.812); DATUM.A.at(2).SetXYZ(303.781,30.307,12.812); DATUM.A.at(3).SetXYZ(426.365,20.519,-13.021);



//Transforming to sensor coordinates
for(Int_t  i=0;i<DATUM.Num_Entries;i++){
//for(Int_t  i=0;i<4;i++){
DATUM.A.at(i)=DATUM.A.at(i) - v2;
x=DATUM.A.at(i)*xplane;
y=DATUM.A.at(i)*yplane;
z=DATUM.A.at(i)*zplane;
DATUM.A.at(i).SetXYZ(x,y,z);
DATUM.A.at(i)=DATUM.A.at(i) + v2;
}
